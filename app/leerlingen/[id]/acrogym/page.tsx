...
const categorieen = ["Inzet", "Techniek", "Ontwerpen"];
...
export default function AcrogymPage({
	params,
}: {
	...
	async function handleSave() {
		...
		const { error } = await supabase.from("acrogym_scores").insert({
			student_id: Number(id),
			inzet: scores.Inzet,
			techniek: scores.Techniek,
			ontwerpen: scores.Ontwerpen,
		});
	...
	useEffect(() => {
		...
		async function loadLeerling() {
			...
			if (isMounted) {
				...
				if (scoreData) {
					const savedScores = scoreData as AcrogymScore;

					setScores({
						Inzet: savedScores.inzet,
						Techniek: savedScores.techniek,
						Ontwerpen: savedScores.ontwerpen,
					});
				}
		...
		<main className="min-h-screen bg-slate-100 p-6 sm:p-10">
			<div className="mx-auto max-w-4xl">
				...
				<div className="mb-6 flex flex-wrap items-baseline justify-between gap-3">
					<h1 className="text-4xl font-bold">Acrogym beoordeling</h1>
					{isLoading ? (
