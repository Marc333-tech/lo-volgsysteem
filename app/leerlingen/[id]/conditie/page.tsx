...
const categorieen = ["Inzet", "Technisch", "Tactisch", "Limieten"];
...
type ConditieScore = {
	...
	limieten: number;
	rsg_run: string | null;
	shuttle_run: string | null;
};

export default function ConditiePage({
	params,
}: {
	...
	const [rsgRunTijd, setRsgRunTijd] = useState("");
	const [shuttleRunNiveau, setShuttleRunNiveau] = useState("");
	...
	async function handleSave() {
		...
		const { error } = await supabase.from("conditie_scores").insert({
			student_id: Number(id),
			inzet: scores.Inzet,
			technisch: scores.Technisch,
			tactisch: scores.Tactisch,
			limieten: scores.Limieten,
			rsg_run: rsgRunTijd || null,
			shuttle_run: shuttleRunNiveau || null,
		});
		...
		async function loadLeerling() {
			const [{ data: leerlingData }, { data: scoreData }] = await Promise.all([
				...
				supabase
					...
					.select(
						"inzet, technisch, tactisch, limieten, rsg_run, shuttle_run",
					)
			...
			if (isMounted) {
				...
				if (scoreData) {
					const savedScore = scoreData as ConditieScore;

					setScores({
						Inzet: savedScore.inzet,
						Technisch: savedScore.technisch,
						Tactisch: savedScore.tactisch,
						Limieten: savedScore.limieten,
					});
					setRsgRunTijd(savedScore.rsg_run ?? "");
					setShuttleRunNiveau(savedScore.shuttle_run ?? "");
				}
		...
		<main className="min-h-screen bg-slate-100 p-6 sm:p-10">
			<div className="mx-auto max-w-4xl">
				...
				<div className="mb-6 flex flex-wrap items-baseline justify-between gap-3">
					<h1 className="text-4xl font-bold">Conditie beoordeling</h1>
					{isLoading ? (
					...
					<div className="grid gap-4 border-t border-slate-200 pt-6 sm:grid-cols-2">
						<label className="text-sm font-semibold text-slate-800">
							...
							<input
								type="text"
								value={rsgRunTijd}
								onChange={(event) => setRsgRunTijd(event.target.value)}
								placeholder="Bijv. 12:34"
								className="mt-2 block w-full rounded-lg border border-slate-300 px-3 py-3 font-normal text-slate-800 outline-none transition focus:border-slate-500 focus:ring-2 focus:ring-slate-500/30"
							/>
						</label>
						<label className="text-sm font-semibold text-slate-800">
							Shuttle Run niveau
							<input
								...
								min="1"
								step="1"
								value={shuttleRunNiveau}
								onChange={(event) => setShuttleRunNiveau(event.target.value)}
								placeholder="Bijv. 8"
								className="mt-2 block w-full rounded-lg border border-slate-300 px-3 py-3 font-normal text-slate-800 outline-none transition focus:border-slate-500 focus:ring-2 focus:ring-slate-500/30"
							/>
						</label>
